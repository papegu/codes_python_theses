...
import pandas as pd
# Ajouter Dropout et L2 Regularization
...
# Nettoyer les noms des colonnes en enlevant les espaces en trop et en les normalisant
...
# Afficher les noms de colonnes après nettoyage pour confirmer
print(df.columns)
...
# La colonne "Culture_Adaptée" ne doit pas faire partie des cibles (y)
y = df[['SOC', 'Biomasse', 'Fertilité']].values  # Correction du nom de colonne
...
def plot_predictions(y_true, y_pred, model_name="Modèle"):
    ...
    for i, prop in enumerate(['SOC', 'Biomasse', 'Fertilité']):
        ...
        plt.plot(y_pred[:, i], label='Forecast', color='red')
        plt.title(f"Property: {prop}")
        plt.xlabel("Samples")
        plt.ylabel("Value")
        plt.legend()
    plt.tight_layout()
    plt.suptitle(f"Comparison of predictions for {model_name}", fontsize=3)
    plt.show()
...
def create_rtcnet_hybrid_model(input_shape):
    ...
    cnn = tf.keras.layers.MaxPooling1D(pool_size=2)(cnn)
    cnn = tf.keras.layers.Dropout(0.3)(cnn)  # Dropout pour aider à réduire le bruit

    # --- Bloc RNN ---
    rnn = tf.keras.layers.SimpleRNN(64, activation='relu', return_sequences=True, 
                                    kernel_regularizer=regularizers.l2(0.01))(cnn)
    rnn = tf.keras.layers.SimpleRNN(64, activation='relu', return_sequences=False)(rnn)
    rnn = tf.keras.layers.Dropout(0.3)(rnn)  # Dropout après les couches RNN

    # --- Bloc Transformer ---
    transformer = tf.keras.layers.MultiHeadAttention(num_heads=4, key_dim=64)(cnn, cnn)
    transformer = tf.keras.layers.GlobalAveragePooling1D()(transformer)
    transformer = tf.keras.layers.Dropout(0.3)(transformer)  # Dropout après transformer
...
# Performance dictionary for comparison
...
for model_name, model in models.items():
    ...
    model_file_path = f"{model_name}_model.h5"
